...
public class CRCGenerator {
	...
	public static int generateCRC(int[] src) {
		int crc = 0;
		for (int i = 0; i < src.length; i++) {
			crc = CRCTable[((crc >> 8) ^ src[i]) & 0xFF] ^ (crc << 8);
		}
		// 只取低16位
		return (crc & 0xFFFF);
	}
	...
	public static void main(String[] args) {
		// test
		int[] src = { 0x30, 0x30, 0x38, 0x30, 0x55, 0x6A, 0xA0 };
		int crc = CRCGenerator.generateCRC(src);
